...
def create_sock(ip_address, port):
...
def create_part_key(my_pub_key, his_pub_key, my_private_key):
    ...
    part_key = gmpy2.powmod(my_pub_key, my_private_key, his_pub_key)
...
if __name__ == "__main__":
    ...
    port = 9999
    server_socket = create_sock(ip_address, port)
    client_socket, client_address = server_socket.accept()
    print('Подключился чудик:', client_address)
    ...
    his_part_key = int(client_socket.recv(4096).decode())
    print('Его частичный ключ - ', his_part_key)

    full_key = create_full_key(his_part_key, private_key, his_public_key)
    print('Full key - ', full_key)

    message = 'МЕНЯ ЗОВУТ БУБА! И МЫ ДЕЛАЕМ ПРОТОКОЛ ДИФИ ХЕЛЛМАНА'
    encrypted_message = encrypt_message(message, full_key)
    client_socket.send(str(encrypted_message).encode())

    his_encrypted_message = client_socket.recv(4096).decode()
